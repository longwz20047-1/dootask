<template>
    ...
    <div class="setting-component-item system-report-fields">
...
<script>
// [CUSTOM:report-channel] Sprint 4 Pass 1 · Task 4.1 + Pass 2 接线
// 系统级（scope=global）上报字段管理。仅管理员可见（system.vue 已限定 admin tab）。
...
export default {
        ...
        return {
            ...
            columns: [
                ...
                },
                {
                    ...
                    key: 'is_builtin',
                    width: 70,
                    render: (h, p) => h('span', p.row.is_builtin ? this.$L('是') : this.$L('否')),
                },
                { title: this.$L('排序'), key: 'sort', width: 70 },
                {
                    ...
                    render: (h, p) => h('div', [
                        ...
                        h('Button', {
                            props: { type: 'text', size: 'small', disabled: !!p.row.is_builtin },
                            style: { color: p.row.is_builtin ? '' : '#f40' },
    ...
    methods: {
        ...
        deleteField(field) {
            if (field.is_builtin) {
                $A.messageWarning(this.$L('内建字段不可删除'));
                return;
            }
            $A.modalConfirm({
                title: this.$L('确认删除'),
